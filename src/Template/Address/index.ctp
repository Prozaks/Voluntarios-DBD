<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Addres'), ['action' => 'add']) ?></li>
    </ul>
</nav>
<div class="address index large-9 medium-8 columns content">
    <h3><?= __('Address') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('ADD_ID') ?></th>
                <th scope="col"><?= $this->Paginator->sort('COM_ID') ?></th>
                <th scope="col"><?= $this->Paginator->sort('ADD_CALLE') ?></th>
                <th scope="col"><?= $this->Paginator->sort('ADD_NUMERO') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($address as $addres): ?>
            <tr>
                <td><?= $this->Number->format($addres->ADD_ID) ?></td>
                <td><?= $this->Number->format($addres->COM_ID) ?></td>
                <td><?= h($addres->ADD_CALLE) ?></td>
                <td><?= $this->Number->format($addres->ADD_NUMERO) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $addres->ADD_ID]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $addres->ADD_ID]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $addres->ADD_ID], ['confirm' => __('Are you sure you want to delete # {0}?', $addres->ADD_ID)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
        </ul>
        <p><?= $this->Paginator->counter() ?></p>
    </div>
</div>
