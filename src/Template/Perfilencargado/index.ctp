<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <link rel="stylesheet" type="text/css" href="css/reset.css">
    <link rel="stylesheet" type="text/css" href="css/fancybox-thumbs.css">
    <link rel="stylesheet" type="text/css" href="css/fancybox-buttons.css">
    <link rel="stylesheet" type="text/css" href="css/fancybox.css">
    <link rel="stylesheet" type="text/css" href="css/animate.css">
    <link rel="stylesheet" type="text/css" href="css/main.css">
    <link rel="stylesheet" type="text/css" href="css/fonts.css">


    <script type="text/javascript" src="js/fancybox.js"></script>
    <script type="text/javascript" src="js/fancybox-buttons.js"></script>
    <script type="text/javascript" src="js/fancybox-media.js"></script>
    <script type="text/javascript" src="js/fancybox-thumbs.js"></script>
    <script type="text/javascript" src="js/wow.js"></script>
    <script type="text/javascript" src="js/main.js"></script>
    <script type="text/javascript" src="js/jquery.js"></script>
    <script type="text/javascript" src="js/arriba.js"></script>

</head>
<body>
<span class="ir-arriba icon-arrow-up"></span>
    <section class="fondoAd final">
        <header class="envoltura final">
            <a href="#home"><img class="logo" src="img/LogoPorAhora.png" alt=""/></a>
            <nav>
                <ul>
                    <li class="page-scroll">
                        <a href="CrearTarea">Definir tareas</a>
                    </li>
                    <li class="page-scroll">
                        <a href="SeleccionarVoluntario">Seleccionar voluntarios</a>
                    </li>
                    <li class="page-scroll">
                        <a href="AsignarVoluntario">Asignar voluntarios</a>
                    </li>
                    <li class="page-scroll">
                        <a href="SelectEmergencia">Modificar estado misión</a>
                    </li>
                </ul>
            </nav>
        </header>

        <div class="caption final animated wow fadeInDown clearfix">
            <h1>¡Bienvenido Encargado</h1>
              <a href="users/logout">Cerrar Sesión</a>
            <hr>
        </div>
        <div class="shadow"></div>
    </section>
</body>
</html>
